...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  do {
    if (empty($id) || empty($name) || empty($email) || empty($phone) || empty($address) || empty($student_id)) {
      $errorMessage = "All the fields are required.";
      break;
    }

    $sql = "UPDATE students SET name = '$name', email = '$email', phone = '$phone', address = '$address', student_id = '$student_id' 
            WHERE id = $id";
    $result = $connection->query($sql);
    ...
    $successMessage = "Form has been updated correctly.";
    header("location: /Student_Registration_Form/index.php");
    exit;
  } while (false);
}
